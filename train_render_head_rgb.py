...
DEFAULT_OUTPUT_DIR = "render_head_rgb_output"
...
def train_render_head(
    ...
    optimizer = torch.optim.Adam(R.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    with open(loss_log_path, "w", newline="") as csvfile:
        ...
        writer.writerow(["轮次", "总损失", "zKL", "方向耦合损失", "比例耦合损失"])
        for row in loss_history:
            writer.writerow(row)

    torch.save(R.state_dict(), os.path.join(output_dir, "render_head.pt"))
